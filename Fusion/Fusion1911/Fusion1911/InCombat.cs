...
namespace Fusion1911
{
    public class InCombat
    {
        ...
        public Return ChangeHealth(UInt64 amount, bool damage)
        {
            if (amount != 0)
            {
                if (damage)
                {
                    ...
                    if (newamount > Health)
                    {
                        Health = 0;
                        CheckDeath();
                        return new Return(string.Format(STRINGS._COMBATLOG_DEALTDAMAGE, Name, newamount, amount - newamount, targetPointer.Name), STRINGS._COMBATLOG_DEALTDAMAGE);
                    }
                    else
                    {
                        Health -= newamount;
                        CheckDeath();
                        return new Return(string.Format(STRINGS._COMBATLOG_DEALTDAMAGE, Name, newamount, amount - newamount, targetPointer.Name), nameof(STRINGS._COMBATLOG_DEALTDAMAGE));
                    }
                }
                else
                {
                    ...
                    {
                        Health = combatStats.Values[6];
                        return new Return(string.Format(STRINGS._COMBATLOG_HEALED, Name, targetPointer.Name, amount), nameof(STRINGS._COMBATLOG_HEALED));
                    }
                    else
                    {
                        Health += amount;
            ...
            return new Return(string.Format(STRINGS._COMBATLOG_MISSED, Name), nameof(STRINGS._COMBATLOG_MISSED));
        }
        ...
        public Return CastAbility(Ability AbilityPointer)
        {
            ...
            if (Dead)
            {
                return new Return(STRINGS._MESSAGE_BAD_DEAD, nameof(STRINGS._MESSAGE_BAD_DEAD));
            }
            if (!(AbilityPointer.EnergyCost <= Energy))
            {
                return new Return(STRINGS._MESSAGE_BAD_NOENERGY, nameof(STRINGS._MESSAGE_BAD_NOENERGY));
            }
            return new Return(STRINGS._MESSAGE_BAD_NOSELECTEDABILITY, nameof(STRINGS._MESSAGE_BAD_NOSELECTEDABILITY));
        }
        public void ChangeTarget(InCombat c)
        {
            targetPointer = c;
        }
        public Return UseConsumable(Item i)
        {
            if (i.IsConsumable && !Dead)
            {
                ...
                if (x.Turns != 255)
                {
                    return new Return(STRINGS._MESSAGE_BAD_NOTCONSUMABLE, nameof(STRINGS._MESSAGE_BAD_NOTCONSUMABLE));
                }
                Buffs.GiveBuff(x, new Stats { Values = new List<ulong> { 1, 1, 1, 1, 1, 1, 1, 1 } });
                return new Return(string.Format(STRINGS._COMBATLOG_CONSUMABLE, Name, i.Name), nameof(STRINGS._COMBATLOG_CONSUMABLE));
            }
            else if (Dead)
            {
                return new Return(STRINGS._MESSAGE_BAD_DEAD, nameof(STRINGS._MESSAGE_BAD_DEAD));
        ...
        public Return CheckDeath()
        {
            if (Health < 1)
            {
                ...
                return new Return(STRINGS._MESSAGE_BAD_YOUDIED, nameof(STRINGS._MESSAGE_BAD_YOUDIED));
            }
            return new Return(STRINGS._GOOD, nameof(STRINGS._GOOD));
        }
        public Return Resurrect()
        {
            if (Dead)
            {
                CalculateStats();
                Dead = false;
                return new Return(STRINGS._MESSAGE_GOOD_YOUGOTRESS, nameof(STRINGS._MESSAGE_GOOD_YOUGOTRESS));
            }
            return new Return(STRINGS._GOOD, nameof(STRINGS._GOOD));
        }
        #endregion
